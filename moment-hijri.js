...
(function (root, factory) {
  ...
  ************************************/
		...
		iMonthsParse: function (monthName) {
			...
			for (i = 0; i < 12; i += 1) {
				...
				if (!this._iMonthsParse[i]) {
					...
					regex =
						'^' + this.iMonths(mom, '') + '$|^' + this.iMonthsShort(mom, '') + '$';
	...
	var iMonthNames = {
		iMonths:
			'Muharram_Safar_Rabiul Awal_Rabiul Akhir_Jumadil Awal_Jumadil Akhir_Rajab_Syaban_Ramadhan_Syawal_Zulqadah_Zulhijah'.split(
				'_'
			),
		iMonthsShort: 'Muh_Saf_Rab-I_Rab-II_Jum-I_Jum-II_Raj_Sya_Ram_Syw_Zul-Q_Zul-H'.split(
			'_'
		),
	};
	...
	if (typeof moment.updateLocale === 'function') {
		moment.updateLocale('id', iMonthNames);
	} else {
		var oldLocale = moment.locale();
		moment.defineLocale('id', iMonthNames);
